$(() => {
  ...
  if (waiveIbanForm.length === 0) {
    return
  }
  const volunteerId = waiveIbanForm
    .find('input[name$="assignment[volunteer_attributes][id]"]')
    .val()
  _(['iban', 'bank']).forEach((fieldName) => {
    waiveIbanForm
      .find(`input[name$="assignment[volunteer_attributes][${fieldName}]"]`)
      .on(
        'input',
        throttle(({ target }) => {
  ...
  waiveIbanForm
    .find('input[name$="assignment[volunteer_attributes][waive]"]')
    .on(
      'change',
      throttle(({ target }) => {
...
const throttle = (callBack, time = window.THROTTLE_TIMEOUT) =>
  _.throttle(callBack, time)

const valueOrChecked = (field) =>
  field.is(':checkbox') ? field.is(':checked') : field.val()
