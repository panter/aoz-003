...
class GroupAssignmentPolicyTest < PolicyAssertions::Test
  test 'superadmin_can_use_all_actions' do
    assert_permit(create(:user), GroupAssignment, *actions_list)
  end
  ...
  test 'department_manager_has_limited_access' do
    ...
    assert_permit(department_manager, GroupAssignment, *actions_list(:terminated_index))
  ...
  test 'social_worker_has_no_access' do
    refute_permit(create(:social_worker), GroupAssignment, *actions_list)
  end
