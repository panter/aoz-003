...
class GroupOfferTest < ActiveSupport::TestCase
  ...
  test 'department manager with no department cannot create group offer' do
    group_offer = GroupOffer.new user: @department_manager, title: 'bogus_title', group_offer_category: @category
    ...
    assert_equal(
      ...
      group_offer.errors.messages[:user_no_department]
    )
  end
  ...
  test 'department manager with wrong department cannot create group offer' do
    @department_manager.department = [@foreign_department]
    @department_manager.save
    group_offer = GroupOffer.new(user: @department_manager, title: 'bogus_title', department: @department,
                                 group_offer_category: @category)
    refute group_offer.valid?
    assert_equal ['Nicht der richtige Standort.'], group_offer.errors.messages[:user_wrong_department]
  end
