...
class VolunteersTest < ApplicationSystemTestCase
  ...
  test 'new volunteer form' do
    ...
    fill_in 'Was ist Ihre Motivation, Freiwilligenarbeit mit '\
            'Migrant/innen zu leisten?',
            with: 'asfd'
    check('volunteer_experience')
    fill_in 'Was erwarten Sie von einer Person, die Sie '\
            'begleiten würden / Ihrem Freiwilligeneinsatz?',
            with: 'asdf'
    fill_in 'Welche Stärken oder Kompetenzen (sozial, beruflich) '\
            'könnten Sie in Ihre Freiwilligenarbeit einbringen?',
            with: 'asdf'
    fill_in 'Welche sind Ihre wichtigsten Freizeitinteressen?',
            with: 'asdf'
    check('Training')
    check('German Course')
    check('Other Offer')
    ...
    check('volunteer_waive')
    check('volunteer_weekend')
    fill_in 'Genauere Angaben',
            with: 'I am every two weeks available on tuesdays asdfasdf.'

    first(:button, 'Freiwillige/n erfassen').click
    assert_text 'Freiwillige/r wurde erfolgreich erstellt.'
  end
  ...
  test 'show volunteer checklist' do
    ...
    check('volunteer_trial_period')
    check('volunteer_intro_course')
    check('volunteer_doc_sent')
    check('volunteer_bank_account')
    check('volunteer_evaluation')

    first(:button, 'Freiwillige/n erfassen').click

    assert_field 'Probezeitbericht erhalten', checked: true
    assert_field 'Einführungskurs besucht', checked: true
    assert_field 'Dossier Freiwillige engagiert verschickt', checked: true
    assert_field 'Kontodaten eingetragen', checked: true
    assert_field 'Abschlussevaluation erhalten', checked: true
  end
  ...
  test 'volunteer checklist has default values (false)' do
    ...
    assert_field 'Probezeitbericht erhalten', checked: false
    assert_field 'Einführungskurs besucht', checked: false
    assert_field 'Dossier Freiwillige engagiert verschickt', checked: false
    assert_field 'Kontodaten eingetragen', checked: false
    assert_field 'Abschlussevaluation erhalten', checked: false
  end

  test 'rejection fields are shown only when the volunteer is rejected' do
    visit new_volunteer_path
    assert_text 'Freiwillige/n erfassen'
    refute_text 'Grund für die Ablehnung', wait: 0
    refute_field 'Erläuterung zur Ablehnung', wait: 0
    ...
    refute_text 'Grund für die Ablehnung', wait: 0
    refute_text 'Erläuterung zur Ablehnung', wait: 0
  ...
  test 'volunteer has no secondary phone field' do
    ...
    assert_text Volunteer.first.contact.full_name
    refute_field 'Telefonnummer 2', wait: 0
  end
  ...
  test 'volunteer_experience_description_field_is_conditional' do
    visit new_volunteer_path
    assert_text 'Freiwillige/n erfassen'
    refute_text 'Falls Sie bereits Erfahrungen mit Freiwilligenarbeit haben, '\
                'bitte diese genauer erläutern.',
                wait: 0
    check('volunteer_experience')
    wait_for_ajax
    assert_text 'Falls Sie bereits Erfahrungen mit Freiwilligenarbeit haben, '\
                'bitte diese genauer erläutern.'
    uncheck('volunteer_experience')
    wait_for_ajax
    assert_field 'volunteer_experience'
    refute_text 'Falls Sie bereits Erfahrungen mit Freiwilligenarbeit haben, '\
                'bitte diese genauer erläutern.',
                wait: 0
  end
  ...
  test 'volunteer_pagination' do
    ...
    Volunteer.order('acceptance asc').paginate(page: 2).each do |volunteer|
      assert_text "#{volunteer.contact.full_name} #{volunteer.contact.city}"\
        " #{volunteer.contact.postal_code}"
    end

    within page.first('.pagination') do
      assert_link '1', href: volunteers_path(page: 1)
      assert_link 'Zurück', href: volunteers_path(page: 1)
    end
  end
  ...
  test 'volunteer (with assignments) index partial has no journal link' do
    ...
    visit volunteer_path(volunteer)
    assert_css '.assignments-table' # only here to avoid waiting with refute
    within '.assignments-table' do
      refute_link 'Journal', wait: 0
    end
  end
  ...
  test 'accepted at creation volunteer gets invited' do
    ...
    assert_text 'Freiwillige/r wurde erfolgreich erstellt. '\
                'Einladung wurde an volunteer@aoz.ch verschickt.'
  ...
  test 'imported_create_account_for_imported_volunteer' do
    ...
    import = Import.create(base_origin_entity: 'tbl_Personenrollen',
                           access_id: 1,
                           importable: volunteer,
                           store: { haupt_person: { email: 'imported@example.com' } })
    visit volunteers_path
    assert_text 'Kein Login'
    assert_text 'Importiert'
    click_link 'Anzeigen', href: volunteer_path(volunteer)
    assert_text "Für die Emailadresse #{import.email} einen Account erstellen"
    assert_field 'Mailadresse', with: import.email
    click_button 'Einladung an angegebene E-Mail verschicken'
    assert_text 'Freiwillige/r erhält eine Accountaktivierungs-Email.'
  end
  ...
  test 'imported_create_account_with_invalid_imported_email' do
    ...
    visit volunteer_path(volunteer)
    assert_text 'Scheinbar ist die importierte Mailadresse nicht gültig.'
  ...
  test 'imported_create_account_no_email_imported_enter_inavalid_email' do
    ...
    visit volunteer_path(volunteer)
    assert_text 'Es wird eine gültige Emailadresse des Freiwilligen benötigt, um einen'
    assert_empty find_field('Mailadresse').value
    fill_in 'Mailadresse', with: 'invalid'
    click_button 'Einladung an angegebene E-Mail verschicken'
    within '.alert.alert-danger.alert-dismissible' do
      assert_text 'Die Mailadresse ist scheinbar nicht gültig'
      assert_link 'Mailadresse konfigurieren', href: edit_volunteer_path(volunteer)
    end
  end
  ...
  test 'imported volunteer can be invited more than once' do
    ...
    click_link 'Anzeigen', href: volunteer_path(volunteer)
    assert_text 'User Account erstellen'
    assert_text "Für die Emailadresse #{import.email} einen Account erstellen"
    assert_field 'Mailadresse', with: import.email
    refute_css '#reinvite_field', visible: false, wait: 0
    ...
    assert_text "Für die Emailadresse #{import.email} einen Account erstellen"
    assert_field 'Mailadresse', with: import.email
    assert_css '#reinvite_field', visible: false
    assert_equal 1, ActionMailer::Base.deliveries.size
    ...
    assert_text 'User Account erstellen'
    assert_text 'Freiwillige/r erhält eine Accountaktivierungs-Email.'
    assert_text "Für die Emailadresse #{new_address} einen Account erstellen"
    assert_field 'Mailadresse', with: new_address
    assert_css '#reinvite_field', visible: false
    assert_equal 1, ActionMailer::Base.deliveries.size
    ...
    volunteer.reload.user.update(invitation_accepted_at: Time.zone.now)
    visit edit_volunteer_path(volunteer)
    assert_text volunteer.contact.full_name
    refute_text 'User Account erstellen', wait: 0
    refute_text "Für die Emailadresse #{new_address} einen Account erstellen", wait: 0
    refute_css '#reinvite_field', visible: false, wait: 0
  end
  ...
  test 'not resigned volunteer can not be terminated via acceptance select in edit' do
    ...
    assert_text @undecided.contact.full_name
    assert_not page.has_select? 'Beendet', wait: 0

    visit edit_volunteer_path(@invited)
    assert_text @invited.contact.full_name
    assert_not page.has_select? 'Beendet', wait: 0

    visit edit_volunteer_path(@accepted)
    assert_text @accepted.contact.full_name
    assert_not page.has_select? 'Beendet', wait: 0

    visit edit_volunteer_path(@rejected)
    assert_text @rejected.contact.full_name
    assert_not page.has_select? 'Beendet', wait: 0
  end
  ...
  test 'resigned volunteers acceptance can not be changed in edit anymore' do
    ...
    assert_field 'Prozess', disabled: true
  end
  ...
  test 'external volunteer can not get machted with a client' do
    ...
    assert_text @external.contact.full_name
    refute_button 'Klient/in suchen', wait: 0
  ...
  test 'department_manager_can_see_acceptance_manipulation_on_his_volunteers_edit' do
    ...
    login_as department_manager
    visit edit_volunteer_path(volunteer)
    assert_text 'Aufnahme Verwaltung'
  ...
  test 'superadmin can change department of volunteer' do
    ...
    assert_text 'Freiwillige/r wurde erfolgreich aktualisiert.'
    assert_equal volunteer.reload.department, department
  end
  ...
  test 'department_manager can edit volunteer assigned to her '\
       'department with secondary department' do
    volunteer = Volunteer.last
    department = create :department
    department_manager = create :department_manager, department: [department]
  ...
  test 'department_manager can take over volunteer with acceptance undecided' do
    ...
    Volunteer.where.not(acceptance: :undecided).each do |decided|
      decided.update department: department
    end
    ...
    assert_text volunteer.to_s
    assert_text 'Standort'
    assert_text 'Prozess'
    ...
    assert_not page.has_selector? volunteer_selector, wait: 0
  ...
  test 'department is automatically set for department_manager when creating volunteer' do
    ...
    assert_text I18n.t('volunteer_created')
  ...
  test 'invite_imported_volunteer_as_user' do
    ...
    visit volunteer_path(volunteer)
    assert_text 'User Account erstellen'
    assert_field 'Mailadresse', with: volunteer.import.email
    click_button 'Einladung an angegebene E-Mail verschicken'
    assert_text 'Freiwillige/r erhält eine Accountaktivierungs-Email.'
    volunteer.reload
